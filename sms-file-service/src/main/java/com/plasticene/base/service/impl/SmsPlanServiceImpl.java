...
public class SmsPlanServiceImpl implements SmsPlanService {
     ...
     *
     * 执行延时任务方案大概有三种：
     * ① 定时扫描：启定时任务每隔一个时间段扫描一次，查询已经过了执行时间的任务进行执行，这里的扫描可以扫数据库或者redis，
     ...
     * ② 监听redis key过期：我们可以通过计算任务定时执行时间，就是我们存入redis key的过期时间，这样我们就可以通过redis的消息
     *                     发布订阅功能去执行任务了
     ...
     * ③ 消息队列之死信队列：当消息在一个队列中变成死信(dead message)之后，它能被重新发送到另一个交换机中，这个交换机就是DLX，绑定DLX的
